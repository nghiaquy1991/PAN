...
void macApp(macCmd_t *pMsg)
{
  ...
  case MAC_INIT_BEACON_COORD:
    MAC_InitBeaconCoord();
    dealloc = TRUE;
    break;

  case MAC_INIT_BEACON_DEVICE:
  ...
  case MAC_SET_REQ:
    if (pMsg->setParam.paramID == MAC_BEACON_PAYLOAD)
    {
      /* Store the payload locally */
      osal_memcpy(macBeaconPayload, pMsg->setParam.paramValue,
                  MAC_PIB_MAX_BEACON_PAYLOAD_LEN);
      pMsg->setParam.paramValue = macBeaconPayload;
    }
    pMsg->setParam.hdr.status = MAC_MlmeSetReq(pMsg->setParam.paramID, pMsg->setParam.paramValue);
    /* send message to App */
    sendMsg = TRUE;
    dealloc = FALSE;
    break;
  ...
  case MAC_GET_REQ:
    ...
    {
      uint8 len;
      MAC_MlmeGetReq(MAC_BEACON_PAYLOAD_LENGTH, &len);

      pMsg->getParam.len = len;
      if (len > 0)
      {
        pMsg->getParam.pValue = (uint8 *)osal_mem_alloc(len);
        if (pMsg->getParam.pValue)
        {
          osal_memcpy(pMsg->getParam.pValue, macBeaconPayload, len);
        }
        else
        {
          pMsg->getParam.len = 0;
          pMsg->getParam.hdr.status = MAC_NO_RESOURCES;
        }
      }
      else
      {
        pMsg->getParam.pValue = NULL;
      }
    }
    else
    {
      pMsg->getParam.len = MAC_MlmeGetReqSize( pMsg->getParam.paramID );
      pMsg->getParam.pValue = (uint8 *)osal_mem_alloc( pMsg->getParam.len);
      if( pMsg->getParam.pValue)
      {
        pMsg->getParam.hdr.status = MAC_MlmeGetReq(pMsg->getParam.paramID, pMsg->getParam.pValue);
      }
      else
      {
        pMsg->getParam.hdr.status = MAC_NO_RESOURCES;
  ...
  case MAC_SET_SECURITY_REQ:
#ifdef FEATURE_MAC_SECURITY
    pMsg->setParam.hdr.status = MAC_MlmeSetSecurityReq(pMsg->setParam.paramID,
                                                       pMsg->setParam.paramValue);
#else
    pMsg->setParam.hdr.status = MAC_UNSUPPORTED_SECURITY;
#endif
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_GET_SECURITY_REQ:
  {
#ifdef FEATURE_MAC_SECURITY
    uint8 *pInPtr = pMsg->getParam.pValue;
    pMsg->getParam.len = MAC_MlmeGetSecurityReqSize( pMsg->getParam.paramID );
    pMsg->getParam.pValue = (uint8 *)osal_mem_alloc( pMsg->getParam.len);
    if( pMsg->getParam.pValue)
    {
      if (pInPtr != NULL)
      {
        osal_memcpy(pMsg->getParam.pValue, pInPtr, pMsg->getParam.len);
      }
      pMsg->getParam.hdr.status = MAC_MlmeGetSecurityReq(pMsg->getParam.paramID,
                                                         pMsg->getParam.pValue);
    }
    else
    {
      pMsg->getParam.hdr.status = MAC_NO_RESOURCES;
...
#else
    pMsg->getParam.hdr.status = MAC_UNSUPPORTED_SECURITY;
#endif
  ...
  break;
    ...
    case MAC_GET_SECURITY_PTR_REQ:
#ifdef FEATURE_MAC_SECURITY
    if( pMsg->getPtrParam.pValue)
    {
      pMsg->getPtrParam.hdr.status = MAC_MlmeGetPointerSecurityReq(pMsg->getPtrParam.paramID,
                                                         (void **)pMsg->getPtrParam.pValue);
    }
    else
    {
      pMsg->getPtrParam.hdr.status = MAC_NO_RESOURCES;
    }
#else
    pMsg->getPtrParam.hdr.status = MAC_UNSUPPORTED_SECURITY;
#endif
    sendMsg = TRUE;
    dealloc = FALSE;
    break;
  ...
  case MAC_RESET_REQ:
    pMsg->resetReq.status = MAC_MlmeResetReq(pMsg->resetReq.setDefaultPib);
    sendMsg = TRUE;
    dealloc = FALSE;
    break;
  ...
  case MAC_SRC_MATCH_ACK_ALL_PENDING:
    // TODO: No longer supported
    dealloc = TRUE;
    break;

  case MAC_SRC_MATCH_CHECK_ALL_PENDING:
    // TODO: No longer supported
    dealloc = TRUE;
    break;

  case MAC_SET_RADIO_REG:
    /* The MAC_SetRadioRegTable ( pMsg->setRadioTable.txPwrTblIdx, pMsg->setRadioTable.rssiAdjIdx )
     * is not used in CC13xx. Keep this case for backward compatibility purpose.
     */
    dealloc = TRUE;
    break;

  case MAC_UPDATE_PANID:
#ifdef FEATURE_MAC_SECURITY
    macUpdatePanId(pMsg->updatePanId.panId);
#endif
    dealloc = TRUE;
    break;

  case MAC_MCPS_DATA_REQ:
    MAC_McpsDataReq(&pMsg->mcpsDataReq);
    dealloc = FALSE;
    break;

  case MAC_MCPS_DATA_ALLOC:
    pMsg->mcpsDataAlloc.pDataReq = MAC_McpsDataAlloc
      (pMsg->mcpsDataAlloc.len,
       pMsg->mcpsDataAlloc.securityLevel,
       pMsg->mcpsDataAlloc.keyIdMode,
       pMsg->mcpsDataAlloc.includeFhIEs,
       pMsg->mcpsDataAlloc.payloadIeLen);

    if(pMsg->mcpsDataAlloc.pDataReq == NULL)
    {
      pMsg->mcpsDataAlloc.hdr.status = MAC_NO_RESOURCES;
    }
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_MCPS_PURGE_REQ:
    MAC_McpsPurgeReq(pMsg->purgeReq.msduhandle);
    dealloc = TRUE;
    break;

  case MAC_MLME_ASSOCIATE_REQ:
    MAC_MlmeAssociateReq(&pMsg->associateReqEvt.associateReq);
    dealloc = TRUE;
    break;

  case MAC_MLME_ASSOCIATE_RSP:
    pMsg->associateRspEvt.hdr.status =
    MAC_MlmeAssociateRsp(&pMsg->associateRspEvt.associateRsp);
    dealloc = TRUE;
    break;

  case MAC_MLME_DISASSOCIATE_REQ:
    MAC_MlmeDisassociateReq(&pMsg->disassociateReqEvt.disAssociateReq);
    dealloc = TRUE;
    break;

  case MAC_MLME_ORPHAN_RSP:
    MAC_MlmeOrphanRsp(&pMsg->orphanRspEvt.orphanRsp);
    dealloc = TRUE;
    break;

  case MAC_MLME_POLL_REQ:
    MAC_MlmePollReq(&pMsg->pollReqEvt.pollReq);
    dealloc = TRUE;
    break;

  case MAC_MLME_SCAN_REQ:
    MAC_MlmeScanReq(&pMsg->scanReqEvt.scanReq);
    dealloc = TRUE;
    break;

  case MAC_START_REQ:
    MAC_MlmeStartReq(&pMsg->startReqEvt.startReq);
    dealloc = TRUE;
    break;

  case MAC_SYNC_REQ:
    MAC_MlmeSyncReq(&pMsg->syncReqEvt.syncReq);
    dealloc = TRUE;
    break;

   case MAC_RANDOM_BYTE:
    pMsg->randomByte.randByte = MAC_RandomByte();
  ...
  case MAC_YIELD_REQ:
    pMsg->hdr.status = MAC_YieldReq();
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_MSG_DEALLOCATE:
    mac_msg_deallocate((uint8**)&pMsg);
    dealloc = FALSE;
    break;

  case MAC_FH_SET_REQ:
    pMsg->hdr.status = MAC_MlmeFHSetReq(pMsg->fhSetParam.paramID, pMsg->fhSetParam.paramValue);
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_FH_GET_REQ:
    pMsg->fhGetParam.len = MAC_MlmeFHGetReqSize( pMsg->fhGetParam.paramID );
    if ( 0 == pMsg->fhGetParam.len )
    {
      pMsg->hdr.status = MAC_UNSUPPORTED;
      sendMsg = TRUE;
      dealloc = FALSE;
      break;
    }

    pMsg->fhGetParam.pValue = (uint8 *)osal_mem_alloc( pMsg->fhGetParam.len);
    if( pMsg->fhGetParam.pValue)
    {
      pMsg->hdr.status = MAC_MlmeFHGetReq(pMsg->fhGetParam.paramID, pMsg->fhGetParam.pValue);
    }
    else
    {
  ...
  case MAC_MLME_WS_ASYNC_REQ:
    MAC_MlmeWSAsyncReq(&pMsg->asyncReqEvt.asyncReq);
    dealloc = TRUE;
    break;

  case MAC_SEC_ADD_DEVICE:
    pMsg->hdr.status = macWrapperAddDevice(
                    pMsg->secAddDevice.panId,
                    pMsg->secAddDevice.shortAddr,
                    pMsg->secAddDevice.extAddr,
                    pMsg->secAddDevice.exempt,
                    pMsg->secAddDevice.keyIdLookupDataSize,
                    pMsg->secAddDevice.keyIdLookupData,
                    pMsg->secAddDevice.frameCounter,
                    pMsg->secAddDevice.uniqueDevice,
                    pMsg->secAddDevice.duplicateDevFlag);
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_SEC_DEL_DEVICE:
    pMsg->hdr.status = macWrapperDeleteDevice(pMsg->secDelDevice.extAddr);
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_SEC_DEL_KEY_AND_DEVICES:
    pMsg->hdr.status = macWrapperDeleteKeyAndAssociatedDevices(pMsg->secDelKeyAndDevices.keyIndex);
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_SEC_DEL_ALL_DEVICES:
    pMsg->hdr.status = macWrapperDeleteAllDevices();
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_SEC_GET_DEFAULT_SOURCE_KEY:
    pMsg->hdr.status = macWrapperGetDefaultSourceKey(pMsg->secGetDefaultSrcKey.keyId,
                                                     &pMsg->secGetDefaultSrcKey.frameCounter);
    sendMsg = TRUE;
    dealloc = FALSE;
    break;

  case MAC_SEC_ADD_KEY_INIT_FC:
    {
      uint8 lookupList[MAC_MAX_KEY_LOOKUP_LEN+2];

      lookupList[0] = 1;
      lookupList[1] = pMsg->secAddKeyInitFC.lookupDataSize;
      osal_memcpy( &lookupList[2], pMsg->secAddKeyInitFC.lookupData, MAC_MAX_KEY_LOOKUP_LEN);

      pMsg->hdr.status = macWrapperAddKeyInitFCtr(pMsg->secAddKeyInitFC.key,
                                                  pMsg->secAddKeyInitFC.frameCounter,
                                                  pMsg->secAddKeyInitFC.replaceKeyIndex,
                                                  pMsg->secAddKeyInitFC.newKeyFlag,
                                                  lookupList);
      sendMsg = TRUE;
      dealloc = FALSE;
    }
