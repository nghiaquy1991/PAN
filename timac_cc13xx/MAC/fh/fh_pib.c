...
#define FH_PIB_ID_END           FHPIB_NEIGHBOR_VALID_TIME
...
MAC_INTERNAL_API FHAPI_status FHPIB_set(uint16_t fhPibId, void *pData)
{
    ...
    {
        FHUTIL_updateUcNumChannels((uint8_t *)pData);
    }

    if(fhPibId == FHPIB_BC_EXCLUDED_CHANNELS)
    {
        FHUTIL_updateBcNumChannels((uint8_t *)pData);
    }
